...
from .utils_hessian import group_product, group_add, normalization, get_params_grad, hessian_vector_product, orthnormal
...
class hessian():
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, device=None):
        """
    ...
    def dataloader_hv_product(self, v):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.data):
            ...
            if batch_idx>8 and self.full_dataset:
                break
    ...
    def eigenvalues(self, maxIter=100, tol=1e-3, top_n=1):
        ...
        while computed_dim < top_n:
            ...
            for i in range(maxIter):
                print('Computing Hessian: Iter {}'.format(i))
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        for i in range(maxIter):
            ...
            diag.append([torch.sum(x * y).item() for (x, y) in zip(Hv, v)])
            if abs(np.mean(trace_vhv) - trace) / (trace + 1e-6) < tol:
                return trace_vhv, np.mean(diag, axis=0)
            else:
                trace = np.mean(trace_vhv)

        return trace_vhv, np.mean(diag, axis=0)

    def density(self, iter=10, n_v=1):
        """
